...
async def _(event):
    ...
    packname = f"{user.first_name}'s @surajit1 Vol.{pack}"
    packshortname = f"vol_{pack}_with_{userid}"
    await event.edit("`Please wait,it's a sticker!`**（　ﾟДﾟ）**\n`Wait, lemme purchase this sticker over hehe`**ヽ༼ ಠ益ಠ ༽ﾉ**")

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "Surajit_as.png"
    ...
    if is_a_s:
        ...
        #if userid == 719877937:
        #    packshortname = "Surajit_Animated"
        #else:
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if not response.text.startswith("Alright!"):
                if "unacceptable" in response.text:
                    packname = f"{user.id}'s @surajit1 Vol.{pack}"
                    response = await silently_send_message(bot_conv, packname)
                else:
                    await event.edit(f"**FAILED**! @Stickers replied: {response.text}")
    ...
    await event.edit(f"**PURCHASE SUCCESSFULLY** `Its mine now from today` [its here](t.me/addstickers/{packshortname}), pack{pack}"
                     f" `by` {DEFAULTUSER}\n**ᕦ(ò_óˇ)ᕤ**")
